...
use crate::realtime_data::ItemData;
use crate::GainRTData;
    ...
    pub fn parse(input: &str) -> IResult<&str, ItemData> {
        ...
        match NaiveDateTime::parse_from_str(resp[30], "%Y%m%d%H%M%S") {
            ...
            Err(e) => {
                ...
                warn!("parse datatime error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
            }
        };
        ...
        rt_data.turnover = match resp[38].parse::<f32>() {
            Ok(v) => Some(v),
            Err(e) => {
                warn!("parse turnover error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.pe = match resp[39].parse::<f32>() {
            ...
            Err(e) => {
                warn!("parse pe error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.pb = match resp[46].parse::<f32>() {
            ...
            Err(e) => {
                warn!("parse pb error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.traded_market_value = match resp[44].parse::<f32>() {
            ...
            Err(e) => {
                warn!("parse traded_market_value error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.market_value = match resp[45].parse::<f32>() {
            ...
            Err(e) => {
                warn!("parse market_value error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.quantity_relative_ratio = match resp[49].parse::<f32>() {
            Ok(v) => Some(v),
            Err(e) => {
                warn!("parse quantity_relative_ratio error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.entrust_different = match resp.get(50) {
            Some(&v) => {
                match v.parse::<f32>() {
                    Ok(v) => Some(v),
                    Err(e) => {
                        warn!("parse entrust_different error: {}. code: {}, name: {}", e, rt_data.code, rt_data.name);
                        None
                    }
                }
            },
            None => {
                info!("no entrust_different in data. code: {}, name: {}", rt_data.code, rt_data.name);
                None
            },
        };
        ...
        rt_data.average_price = match resp.get(51) {
            ...
            None => {
                info!("no average_price in data. code: {}, name: {}", rt_data.code, rt_data.name);
                None
            },
        };
...
mod tests {
    ...
    fn test_parse_data() {
        ...
        match Tencent::parse(parse1) {
            Ok((_, rt_data)) => {
                ...
                assert_eq!(rt_data.datatime, Some(NaiveDateTime::parse_from_str("20220419161403", "%Y%m%d%H%M%S").unwrap()));
    ...
    #[test]
    fn test_parse_data2() {
        let data = "v_sh518801=\"1~国泰申赎~518801~2.229~2.229~0.000~0~0~0~0.000~0~0.000~0~0.000~0~0.000~0~0.000~0~0.000~0~0.000~0~0.000~0~0.000~0~0.000~0~~20151224150221~0.000~0.00~0.000~0.000~2.230/0/0~0~0~~~~0.000~0.000~0.00~~~0.000~2.452~2.006~\"";
        match Tencent::parse(data) {
            Ok((_, rt_data)) => {
                ...
                assert_eq!(rt_data.datatime, Some(NaiveDateTime::parse_from_str("20151224150221", "%Y%m%d%H%M%S").unwrap()));
